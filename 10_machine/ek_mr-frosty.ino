...
int stepYPin = 18;
int dirYPin = 19;
const int stepPin_extrude= 14;
const int dirPin_extrude = 27;
...
<html>
  ...
  <body>
    ...
    <script>
      ...
      function someShape() {
        let coords = {x: [504, 515, 527, 538, 548, 558, 569, 579, 589, 602, 599, 577, 555, 536, 531, 525, 521, 538, 560, 582, 604, 625, 647, 668, 678, 675, 663, 646, 625, 604, 582, 594, 616, 636, 652, 661, 660, 645, 623, 608, 597, 587, 577, 567, 556, 547, 541, 522, 727, 706, 686, 674, 676, 688, 684, 691, 707, 727, 749, 767, 775, 772, 763, 751, 741, 739, 749, 767, 788, 809, 821, 812, 806, 808, 791, 769, 752, 748, 757, 770, 784, 793, 792, 781, 764, 744, 824, 818, 832, 846, 861, 875, 889, 902, 890, 868, 848, 836, 846, 860, 882, 904, 926, 918, 904, 890, 876, 862, 849, 837, 930, 914, 910, 913, 920, 930, 944, 961, 981, 1002, 1011, 1010, 1005, 997, 984, 968, 950, 956, 973, 984, 992, 996, 994, 980, 959, 944, 934, 928, 925, 930], y:[445, 435, 417, 398, 378, 359, 340, 321, 301, 284, 280, 282, 286, 295, 315, 312, 291, 279, 276, 276, 276, 276, 277, 284, 303, 325, 343, 356, 365, 368, 368, 362, 360, 352, 338, 318, 296, 282, 282, 297, 316, 335, 355, 374, 393, 413, 434, 436, 450, 446, 436, 418, 397, 379, 398, 418, 433, 440, 440, 429, 409, 387, 367, 349, 329, 308, 288, 276, 271, 276, 292, 312, 312, 291, 278, 279, 292, 312, 332, 349, 367, 387, 408, 427, 440, 448, 439, 430, 414, 397, 380, 364, 347, 329, 320, 320, 326, 342, 323, 309, 309, 309, 308, 322, 339, 356, 373, 390, 407, 425, 441, 429, 408, 386, 365, 346, 329, 315, 307, 312, 332, 353, 375, 395, 413, 427, 439, 430, 417, 398, 377, 356, 334, 319, 323, 339, 358, 379, 401, 422]
};
        console.log(JSON.stringify({mode:"ice", xy: coords}));
        websocket.send(JSON.stringify({mode:"ice", xy: coords}));
      }
      ...
      function generateCircleXY() {
        ...
        const x_quart = x_vals.slice(0, parseInt(x_vals.length * 0.25));
        const y_quart = y_vals.slice(0, parseInt(y_vals.length * 0.25));
        console.log(x_quart, y_quart);
        x_vals.push(x_vals[0], 550 * 2.089 * +data.cx);
        y_vals.push(y_vals[0], 550 * +data.cy);
...
void setup() {
  ...
  pinMode(dirPin_extrude, OUTPUT);
  pinMode(stepPin_extrude, OUTPUT);
  digitalWrite(dirPin_extrude, HIGH);
...
void icing() {
  ...
  if (extruding) {
    digitalWrite(stepPin_extrude, HIGH);
  }
  else {
    digitalWrite(stepPin_extrude, LOW);
  }
